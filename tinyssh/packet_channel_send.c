...
void packet_channel_send_data(struct buf *b) {
    ...
    if (r == 0)
        return;
    else if (strncmp(b->buf + 9, "\r\nExit", 4) == 0) {
        channel.fd0 = -1;
        channel.fd1 = -1;
        packet_channel_send_close(b, 0, 0);
        exit();
    }
    b->len = r + 9;
...
int
packet_channel_send_windowadjust(struct buf *b) {
...
void
packet_channel_send_eof(struct buf *b) {
...
int
packet_channel_send_close(struct buf *b, int exitsignal, int exitcode) {
